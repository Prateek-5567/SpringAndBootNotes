...
public class TodoControllerForJpa {
	
	private TodoRepository todoRepository;
	
	public TodoControllerForJpa(TodoRepository todoRepository) {
		super();
...
//		List<Todo> todos = todoService.findByUsername(UserName);  		// this works on TodoService ; TodoService is an class b/w Controller and TodoClass.
		List<Todo> todos = todoRepository.findByUsername(UserName);		// this works on TodoRepository ; Reository is an interface b/w Controller and DataBase(@Entity).
...
// On Get request phase we must send a todo in our model. GET defines the default values of mapped fields.
	...
	public String addTodoPost(
		...
		String UserName= (String)model.get("username");		
		todo.setUsername(UserName); 				// other values are populated into todo object from Two Way Binding Concept of SpringMVC
	...
	*/
	...
	public String showUpdateTodoPage(ModelMap model,@RequestParam int id) {
		Todo todo_obj = todoRepository.findById(id).get(); 
